...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    ${CONTAINER_CLI} run --rm -d --name peer1org3_${CC_NAME}_ccaas  \
                  --network fabric_brahma \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \  
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
...
installChaincode 1 0                                # Passing arguments with sequence OrgNo PeerNo
infoln "Installing chaincode on peer1.org1..."
installChaincode 1 1                                # Passing arguments with sequence OrgNo PeerNo
infoln "Install chaincode on peer0.org2..."
installChaincode 2 0                                # Passing arguments with sequence OrgNo PeerNo
infoln "Installing chaincode on peer1.org2..."
installChaincode 1 1                                # Passing arguments with sequence OrgNo PeerNo
infoln "Install chaincode on peer0.org3..."
installChaincode 3 0                                # Passing arguments with sequence OrgNo PeerNo
infoln "Installing chaincode on peer1.org3..."
installChaincode 3 1                                # Passing arguments with sequence OrgNo PeerNo
