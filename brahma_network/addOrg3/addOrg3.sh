...
function generateOrg3() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    echo "i am in Certificate Authorities"
...
function generateOrg3Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.brahma.com/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  # start org3 nodes
  # echo "container CLI is $CONTAINER_CLI ##########################################################################################"
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org3.brahma.com/peers/peer0.org3.brahma.com/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d ../organizations/ordererOrganizations ]; then
    fatalln "ERROR: Please, run ./brahma.sh up createChannel first."
  fi

  # generate artifacts if they don't exist
  if [ ! -d "../organizations/peerOrganizations/org3.brahma.com" ]; then
    generateOrg3
