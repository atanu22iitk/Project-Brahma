...
function networkUp() {
  checkPrereqs
  echo "Prereqs are valid....Validation successful."
  # generate artifacts if they don't exist
  if [ ! -d "organizations/peerOrganizations" ]; then
    echo "Starting with createOrgs function"
    createOrgs
  fi
  ...
  echo "network is now Up check using docker ps -a"
}
...
function networkDown() {
  ...
  # remove channel artifacts by tearing down the channel
  ###################################### Add your code here to tear down existing channels run command peer channel list to see how many channels are existing #########
  # peer channel list
  # Don't remove the generated artifacts -- note, the ledgers are always removed
  if [ "$MODE" != "restart" ]; then
    # Bring down the network, deleting the volumes
    ${CONTAINER_CLI} volume rm docker_orderer.brahma.com docker_peer0.org1.brahma.com docker_peer0.org2.brahma.com
