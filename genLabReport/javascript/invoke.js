...
async function main(orgName, hospId, channelName, contractName, functionName, patientId, doctorId, staffId, labReportId, department) {
//async function main(hospId, channelName, contractName, orgName) {
    try {
        ...
        const resData = await contract.submitTransaction(`${functionName}`, `${patientId}`, `${doctorId}`, `${staffId}`, `${labReportId}`, `${department}`, `${hospId}`, `${timeDate}`);
        //await contract.submitTransaction('initLedger', 'initialNullValues');
        console.log(`genLabReport Transaction has been submitted. Got the key as "${resData.toString()}" , save this key for future reference`);
